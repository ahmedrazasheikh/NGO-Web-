...
export default function BasicModal2({ id, Image }) {
  ...
  const [amontCollected, setAmontCollected] = useState();
  ...
  const handleInputChange = (e) => {
    ...
    if (files && files.length > 0) {
      const newImageSrcList = Array.from(files).map((file) =>
  ...
  const sendData = (e) => {
    ...
    axios
      .post(`https://busy-tan-dhole-wig.cyclic.app/api/v1/updated/completed/${id}`, formData, {
        headers: {
...
<div   className="mb-4" >
      ...
      <select id="citySelect" value={selectedCity} onChange={handleCityChange}>
        ...
        <option value="Rato Dero">Rato Dero</option>
          ...
          <input
            onChange={(e) => {
              setAmontCollected(e.target.value);
            }}
            type="text"
            id="name"
            name="name"
